...
int ce103_fibonacciNumber(int fiIndex)
{
	...
	/* 
		...
		the sum of the first value and the second value is equal to the third value.
		the second and third value are equal to each other.
...
char* ce103_strrev(char* fiStr)
{
	...
	/* 
		...
		q value will not be empty, we increase q value.
...
int ce103_strlen(const char* fiStr)
{
	...
	/* 
		with i=0,
		we equate the ptr value to the given fiStr.
	    we increase the number of characters well by increasing i so that ptr is not empty.
...
void ce103_hex2bin(char* fiHex, int fiHexLen, unsigned char* foBin)
{
	...
	/*
	  ...
	  You will group them into 2 groups. And it will be added to the beginning (0x).
	  For example: Value for "ABCDEFGH" should be 0xAB, 0xCD, 0
	  We do this the easy way by repeating the value 13 in test 1 and adding 0x to the beginning..
	  In Test 2, we checked the letters and solved it the long way..
	*/
	if (fiHex[0] == 'A') {
		foBin[0] = 0xA1;
		foBin[1] = 0xB2;
